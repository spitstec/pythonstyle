...
#include "windows.h"
HANDLE txtMutex; 
void txtSetLocking(int lon) {
  if (lon)
    txtMutex = CreateMutex(NULL,FALSE,NULL) ;
  else
    CloseHandle(txtMutex); }
    
#define txtLock() WaitForSingleObject( txtMutex, INFINITE)
#define txtUnlock() ReleaseMutex(txtMutex)
#else
#define txtLock() 
#define txtUnlock() 
void txtSetLocking(int Lon) { }
...
//grab the first part of a string multiple delimiters
...
tXt txtInsert(tXt src,tXt stuf,int pos) {
  pos = limii(pos,0,txtMAXLEN) ;
  return txtConcat(txtSub(src,0,pos),stuf,txtSub(src,pos,txtNOTFOUND),NULL) ;}

// make string filled with same chars
tXt txtFill(char c,int len) {
  len = limii(len,0,txtMAXLEN) ;
  tXt s = txtMalloc(len+1) ;
  s[len--] = 0 ;
  while (len >= 0)  {
    s[len--] = c ; }
  return s ; }
...
void wordfrequency(void) {
  ...
  while (rlin != txtEndFile) {
    tXt lin = txtTrims(rlin = txtFromFile(fi),delims) ;
    while (lin[0]) {
